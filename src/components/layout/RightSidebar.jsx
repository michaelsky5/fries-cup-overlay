...
export default function RightSidebar({ previewScene, showRightColumn, density = 'standard', densityTokens }) {
  ...
  const lastSaveTime = useRef(Date.now()); // 🚀 记录上次保存时间，防止重复触发
  ...
  const snapshotMaxHeight = showRightColumn ? density === 'spacious' ? '300px' : isCompactSidebar ? '210px' : '250px' : density === 'spacious' ? '260px' : isCompactSidebar ? '180px' : '220px';
  const logMaxHeight = showRightColumn ? density === 'spacious' ? '300px' : isCompactSidebar ? '220px' : '250px' : density === 'spacious' ? '240px' : isCompactSidebar ? '180px' : '220px';
  ...
  const handleTakeSnapshot = () => {
    showModal({
      type: 'prompt', title: 'TAKE SNAPSHOT', message: 'Enter snapshot label / note:', placeholder: `e.g. MAP ${matchData.currentMap || 1} POST-MATCH`,
      onConfirm: val => {
  ...
  const handleRestore = snap => {
    showModal({
      type: 'confirm', title: 'RESTORE SNAPSHOT', isDanger: true, message: `[DANGER] Restore [${snap.label}]?\n\nAll current unsaved values will be overwritten.`,
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gap: isCompactSidebar ? '10px' : '12px' }}>
        <ShellPanel title="Data Snapshot & Backup" accent density={density} bodyStyle={{ ...ui.panelBody, padding: isCompactSidebar ? '10px' : ui.panelBody?.padding }}>
          <div style={{ ...ui.stack, gap: sectionGap }}>
            <div style={{ ...ui.inline2, gap: sectionGap }}>
              <button onClick={handleTakeSnapshot} style={{ ...ui.actionBtn, backgroundColor: COLORS.yellow, color: COLORS.black, padding: buttonPad || ui.actionBtn.padding, fontSize: isCompactSidebar ? '11px' : undefined, fontWeight: '900', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', lineHeight: 1 }}>SNAPSHOT</button>
              <button onClick={toggleAutoSave} style={{ ...ui.btn, backgroundColor: autoSaveEnabled ? '#2ecc71' : '#444', color: autoSaveEnabled ? '#fff' : '#888', padding: buttonPad || ui.btn.padding, fontSize: isCompactSidebar ? '11px' : undefined, fontWeight: '900', cursor: 'pointer', transition: 'background-color 0.2s, color 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', lineHeight: 1 }}>AUTOSAVE</button>
            </div>
            ...
            <div className="fc-custom-scroll" style={{ display: 'flex', flexDirection: 'column', gap: sectionGap, maxHeight: snapshotMaxHeight, overflowY: 'auto', paddingRight: '6px' }}>
              {snapshots.length ? snapshots.map(snap => (
                <div key={snap.id} style={{ ...panelBase, padding: cardPadding, flexShrink: 0, borderLeft: snap.label.includes('[AUTO]') ? '2px solid #2ecc71' : `2px solid ${COLORS.yellow}` }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                    <button onClick={() => handleRestore(snap)} style={{ ...ui.outlineBtn, color: COLORS.yellow, borderColor: COLORS.yellow, padding: buttonPad || ui.outlineBtn.padding, fontSize: isCompactSidebar ? '11px' : undefined, fontWeight: '900', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', lineHeight: 1 }}>RESTORE</button>
                    <button onClick={() => updateData({ ...matchData, snapshots: snapshots.filter(s => s.id !== snap.id) })} style={{ ...ui.outlineBtn, color: COLORS.red, borderColor: 'transparent', padding: buttonPad || ui.outlineBtn.padding, fontSize: isCompactSidebar ? '11px' : undefined, fontWeight: '900', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', lineHeight: 1 }}>DELETE</button>
                  </div>
                </div>
              )) : <div style={{ ...panelBase, padding: cardPadding, color: COLORS.faintWhite, fontSize: isCompactSidebar ? '11px' : '12px', textTransform: 'uppercase', letterSpacing: '0.6px' }}>No snapshots yet.</div>}
            </div>
          </div>
        </ShellPanel>

        <ShellPanel title="Live Status" accent density={density} bodyStyle={{ ...ui.panelBody, padding: isCompactSidebar ? '10px' : ui.panelBody?.padding }}>
        ...
        <ShellPanel title="Operation Log" accent density={density} bodyStyle={{ ...ui.panelBody, padding: isCompactSidebar ? '10px' : ui.panelBody?.padding }}>
          <div className="fc-custom-scroll" style={{ display: 'flex', flexDirection: 'column', gap: sectionGap, maxHeight: logMaxHeight, overflowY: 'auto', paddingRight: '6px' }}>
            {history && history.length ? history.slice().reverse().map((entry, idx) => (
              <div key={`${entry.time || idx}-${entry.action || idx}`} style={{ ...panelBase, padding: cardPadding, borderLeft: idx === 0 ? `3px solid ${COLORS.yellow}` : `2px solid rgba(255,255,255,0.12)`, flexShrink: 0 }}>
                ...
                {/* 🚀 修复：将 entry.label 改回 entry.action */}
                <div style={{ fontSize: cardTitleSize, fontWeight: '900', color: COLORS.white, textTransform: 'uppercase', lineHeight: 1.25 }}>{entry.action || 'UNKNOWN ACTION'}</div>
              </div>
            )) : <div style={{ ...panelBase, padding: cardPadding, color: COLORS.faintWhite, fontSize: isCompactSidebar ? '11px' : '12px', textTransform: 'uppercase', letterSpacing: '0.6px' }}>No operation logs yet.</div>}
          </div>
        </ShellPanel>
      </div>
    </>
