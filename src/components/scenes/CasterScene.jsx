...
// 🌟 性能优化：使用 React.memo 防止无关状态更新导致的重绘
const CasterCard = React.memo(({ caster, delay, index, compact = false, multiMode = false }) => {
  const displayName = caster?.label || caster?.id || `CASTER ${index + 1}`;
  const displayTitle = caster?.title || 'COMMENTATOR';
  const displaySocial = caster?.social || `@${(caster?.label || caster?.id || `caster${index + 1}`).replace(/\s+/g, '_')}`;
  const avatarKey = caster?.id || caster?.label || 'COMMENTATOR';
    ...
    return (
      ...
      >
        ...
        >
          <img
            src={imgPath}
            onError={e => { e.target.src = '/assets/casters/OW.png'; }}
            alt={displayName}
            style={{
              width: '100%',
              height: '100%',
        ...
        >
          <div
            style={{
              position: 'absolute',
              inset: 0,
              background: 'repeating-linear-gradient(90deg, rgba(255,255,255,0.018) 0 1px, transparent 1px 22px)',
              opacity: 0.4,
              pointerEvents: 'none'
            }}
          />
    ...
    >
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            pointerEvents: 'none',
            boxShadow: 'inset 0 0 80px rgba(0,0,0,0.16), inset 0 0 0 1px rgba(255,255,255,0.04)'
          }}
        />
        <div
          style={{
      ...
      <div
        style={{
          minWidth: `${cardW}px`,
          width: 'max-content',
          background: `linear-gradient(180deg, ${COLORS.yellow} 0%, #e8b91b 100%)`,
          padding: footerPad,
          boxSizing: 'border-box',
          display: 'flex',
          flexDirection: 'column',
      ...
      >
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: 'repeating-linear-gradient(90deg, rgba(42,42,42,0.05) 0 1px, transparent 1px 22px)',
            pointerEvents: 'none',
            opacity: 0.4
          }}
        />
...
export default function CasterScene({ matchData = {} }) {
  ...
  const casters = useMemo(() => {
    ...
    const castersFromLegacy = [
      {
        id: matchData.caster1 || '',
        label: matchData.caster1Label || '',
        title: matchData.caster1Title || 'COMMENTATOR',
        social: matchData.caster1Social || '',
        avatar: matchData.caster1Avatar || ''
      },
      {
        id: matchData.caster2 || '',
        label: matchData.caster2Label || '',
        title: matchData.caster2Title || 'COMMENTATOR',
        social: matchData.caster2Social || '',
        avatar: matchData.caster2Avatar || ''
      }
    ];
  ...
  return (
    ...
    >
        ...
        <div style={{ position: 'absolute', top: '60px', left: '80px', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
          ...
          <div
            style={{
              width: '420px',
              height: '8px',
              background: `linear-gradient(90deg, ${COLORS.yellow} 0%, rgba(244,195,32,0.14) 100%)`,
      ...
      {displayMode === 'CASTERS' && casterCount > 0 && !isGridMode && (
        <div
          style={{
            ...
            justifyContent: 'center',
            alignItems: 'center',
            gap: casterCount === 1 ? '0' : '140px',
            zIndex: 5,
            position: 'relative'
          }}
        >
          {casters.map((caster, idx) => (
            <CasterCard key={idx} caster={caster} delay={0.35 + idx * 0.18} index={idx} compact={false} />
          ))}
        </div>
      )}
      ...
      {displayMode === 'CASTERS' && casterCount > 0 && isGridMode && (
        ...
        >
          {casters.map((caster, idx) => (
            <CasterCard key={idx} caster={caster} delay={0.22 + idx * 0.08} index={idx} compact multiMode />
          ))}
        </div>
      )}
      ...
      {displayMode === 'CASTERS' && casterCount === 0 && (
        ...
        >
          <div style={{ color: COLORS.white, fontSize: '28px', fontWeight: '900', letterSpacing: '2px', textTransform: 'uppercase' }}>
            No Casters Loaded
          </div>
          <div style={{ color: COLORS.faintWhite, fontSize: '14px', marginTop: '12px', lineHeight: 1.7 }}>
            Please add at least one caster in the control panel.
          </div>
        </div>
      )}
        ...
        >
          <div style={{ ...panelBase, borderTop: `2px solid ${COLORS.yellow}`, padding: '26px 30px' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'grid', gap: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '12px', height: '12px', background: COLORS.yellow }} />
                <span style={{ fontSize: '16px', fontWeight: '900', color: COLORS.softWhite, letterSpacing: '3px', textTransform: 'uppercase' }}>
                  Staff Appreciation
                </span>
              </div>
              <div style={{ fontSize: '64px', fontWeight: '900', color: COLORS.white, lineHeight: 1, letterSpacing: '1px', textTransform: 'uppercase' }}>
                {staffTitle}
              </div>
              <div style={{ fontSize: '22px', fontWeight: '900', color: COLORS.yellow, letterSpacing: '2px' }}>
                {staffSubtitle}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: staffMembers.length > 5 ? '1fr 1fr' : '1fr', gap: '14px', alignContent: 'start' }}>
            {staffMembers.map((member, idx) => (
              <StaffRow key={idx} role={member.role} name={member.name} delay={0.18 + idx * 0.08} index={idx} />
            ))}
            ...
            {!staffMembers.length && (
              ...
              >
                <div style={{ color: COLORS.white, fontWeight: '900', fontSize: '18px', letterSpacing: '2px', textTransform: 'uppercase' }}>
                  No Staff Members
                </div>
                <div style={{ color: COLORS.faintWhite, fontSize: '13px', marginTop: '10px', lineHeight: 1.6 }}>
                  Please add staff members in the control panel.
                </div>
              </div>
            )}
          </div>
